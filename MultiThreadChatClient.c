...
int main(int argc, char *argv[ ])
{
	...
	status = pthread_create(&thread_send, NULL, do_send, (void *)&c_socket);
    //pthread_create with do_receive_chat function
	status = pthread_create(&thread_receive, NULL, do_receive, (void *)&c_socket);
    //pthread_join both threads
	pthread_join(thread_send, (void **)&status);
	pthread_join(thread_receive)
    close(c_socket);
}
