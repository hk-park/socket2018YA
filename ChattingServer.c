    ...
    while(1) {
        ...
        if(res < 0) { //MAX_CLIENT만큼 이미 클라이언트가 접속해 있다면,
            ...
            write(c_socket, greeting, strlen(greeting));
			 pthread_create(&pthread, NULL, do_service, (void *)&c_socket);
        }
    }
}
...
int pushClient(int c_socket) {
	list_c[count+1] = c_socket;
	if(clientCounting > 10)
		return -1;
	else 
		return clientCounting;
}
